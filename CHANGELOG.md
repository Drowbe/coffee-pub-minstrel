...
### Added
- **Architecture documentation.** The module had none: the only prose was a user guide, working notes, and scaffolding inherited from the Coffee Pub starter template. Eight pages now live in `documentation/architecture/` — an overall map (`architecture-minstrel.md`) plus one each for the window and render pipeline, Dashboard, playlists and playback, sound scenes, cues, automation, and storage. They document the invariants, ordering constraints, and the bugs that produced the current shape, rather than restating the code. Verified by: `node tools/check-docs-structure.mjs` passes and `node tools/wiki-sync.mjs build` renders every page with no downgraded links.
- **Five product screenshots** in `documentation/assets/`, WebP, named for the document that owns them: `product-overview.webp` (the README and `home.md` both draw on it) and four `userguide-` captures of the Sound Scenes, Cues, Playlists and Automation tabs. The publisher rewrites their relative paths to raw URLs on the way out, so they render in the repository, in an editor, and on the wiki.
- **`documentation/home.md`, `known-issues.md`, and `userguides/userguide-getting-started.md`.** The getting-started guide is written for a GM at the table and quotes the on-screen setting labels from `lang/en.json`. The screenshots corrected several claims in it that had been derived from source rather than the screen: the tab order is Dashboard, Sound Scenes, Cues, Playlists, Automation rather than the order the code declares them; the third channel readout is labelled **Interface**, not "one-shots and cues"; cues are grouped into **cue sheets**; and the Dashboard's three panes are the Sound Scene, Playlist and Cue **racks**. The remaining claims still need walking in a running world. Known issues records four real defects, including the fixed 2.5s cue duck-restore and the `isPlaying`/`playing` template mismatch.
- **Wiki publishing.** `tools/wiki-sync.mjs`, `tools/check-docs-structure.mjs`, `tools/.gitignore` and `.github/workflows/sync-wiki.yml`, copied byte-identical from Blacksmith and verified with `diff` rather than by eye. Publication is by folder membership rather than a maintained list, and module identity is derived from `module.json`, so nothing in these files is edited per module. Every push to `master` touching `documentation/` mirrors the published folders to the wiki. Minstrel carries no `api/` or `designsystem/` folder: it exposes no API and publishes no design tokens, and under the standard those folders exist only in a module that has something to put in them.
- **`.gitattributes`**, copied from Blacksmith, pinning the text kinds to LF -- including `*.yml`, `.gitignore` and `.gitattributes` itself, without which three of the five copied-unchanged files are unprotected. Verified with `git check-attr eol` rather than by reading the file. Without it, `core.autocrlf=true` (the Windows default, and the setting on this machine) rewrites the copied tool files to CRLF on the next checkout while the hub's stay LF, so a later `diff` reports every line of both tools as changed. Nothing errors and the checker still passes -- the only casualty is the byte-identity check that proves the tools have not been locally patched, and it fails in the direction that looks like a local edit.

### Changed
- **Documentation restructured to the suite-wide standard** (Blacksmith's `global-documentation-standard.md`). `documentation/guides/` dissolved; `user-guide.md` moved to `plans/plan-product-direction.md` because it was a design document rather than a user guide — it specified a recommended data model and product direction, and publishing it as a user guide would have been worse than the gap it filled. The two TODO files (root `TODO.md` and `documentation/todo.md`) merged into `documentation/TODO.md`, dropping the completed items, which live in this file. `plans/plan.md` renamed to `plan-minstrel.md` to match the folder prefix. `README.md` rewritten as a product page carrying the suite's AI-assistance disclosure between `global:ai-assistance` markers.
- **`architecture-window.md` lost its "Open work" section** — a permanent document must not point at the transient lists. The task moved to `TODO.md`; the durable fact it carried (track rows are addressable via `data-value="playlistId::soundId"`, and what row state a targeted updater must reach) moved into the architecture prose so it did not die with the task.
- **Minstrel's code conventions folded into `AGENTS.md`** from the deleted `guides/best-practices.md`, which is where the standard puts them.

### Removed
- **`documentation/guides/blacksmith-apis.md`** — one of five diverging forks of Blacksmith's API notes across the suite. Deleted rather than refiled: folding a fork into a tidier folder preserves the fork. The hub wiki is the source.
- **`documentation/guides/SETUP_PROMPT.md` and `getting-started.md`** — starter-template documents about building a Coffee Pub module, not about Minstrel. Both carried emoji status markers the standard bans.
...
- **`relationships.requires[coffee-pub-blacksmith].compatibility.minimum` pinned to `13.22.0`**, the release that carries `api.geography`. Note that until Blacksmith actually tags 13.22.0, Foundry will refuse to activate Minstrel against an earlier Blacksmith — the pin is a hard requirement, not a warning.
